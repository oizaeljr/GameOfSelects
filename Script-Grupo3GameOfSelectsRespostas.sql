...
SELECT avg(aposta.valor) FROM aposta JOIN batalha ON batalha.idBatalha = aposta.fkAposta_Batalha;
SELECT min(peso), max(peso), avg(peso) FROM pokemon;
SELECT count(idPokedex) FROM pokedex WHERE tipo1 = 'planta' AND tipo2 = 'veneno';
SELECT b.custoReparo, b.dia, e.valor FROM entrada as e JOIN batalha as b ON b.idBatalha = e.fkEntrada_Batalha WHERE tipo = 'Arquibancada' OR tipo = 'Camarote';
SELECT avg(pokemon.peso), avg(pokemon.tamanho), pokedex.tipo1 FROM pokemon JOIN pokedex ON pokemon.fkPokemon_Pokedex = idPokedex GROUP BY pokedex.tipo1;
...
SELECT sum(b.custoReparo), t.nome FROM batalha as b JOIN treinador as t ON b.fkBatalha_Treinador1 = t.idTreinador group by t.nome;
SELECT avg(aposta.valor) AS mediaAposta, batalha.idBatalha FROM aposta JOIN batalha ON batalha.idBatalha = aposta.fkAposta_Batalha GROUP BY batalha.idBatalha ORDER BY mediaAposta DESC LIMIT 2;
